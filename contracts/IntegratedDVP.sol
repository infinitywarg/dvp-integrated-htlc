...
contract IntegratedDVP {
    ...
    event InstructionCreated(bytes32 indexed id, address indexed seller, address indexed buyer, ExtData data);
    ...
    function createSettlement(
        ...
        require(data.encryptedPreimage.length == 64, "malformed encrypted preimage");
    ...
    function executeSettlement(bytes32 id, bytes32 preimage) public returns (bool success) {
        Instruction memory i = instruction[id];
        require(i.seller != address(0), "settlement id doesnt exist");
        require(i.hashlock == keccak256(abi.encode(preimage)), "hashlock invalid");
